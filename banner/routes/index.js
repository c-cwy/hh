...
module.exports.add = async ctx => {
    let { idCard, remark, type, sort, time } = ctx.request.body;
    if (idCard && remark && type && sort && time) {
        ...
        if (cardData.data.length) {
            ...
            try {
                let sql = 'insert into banner(idCard, remark, type, sort, time) values (?,?,?,?,?)';
                let data = await query(sql, [idCard, remark, type, sort, time]);
                ctx.body = {
                    code: 200,
                    msg: '添加成功'
                }
